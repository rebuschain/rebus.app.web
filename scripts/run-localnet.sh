...
docker build --tag rebus/localnet "$DIR"/../localnet

# Make sure that previous container not exist.
docker rm --force rebus_localnet

# Start container as daemon with some ports opening.
docker run -d -p 1317:1317 -p 26657:26657 -p 9090:9090 --name rebus_localnet rebus/localnet
...
echo "Docker container is running on \"rebus_localnet\""
echo "After testing, to remove existing container, run \"sudo docker rm --force rebus_localnet\""
