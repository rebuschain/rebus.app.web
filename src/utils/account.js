...
function normalizePubkey(input) {
	...
	if (typeof input === 'string') {
		return (0, decodeBech32Pubkey)(input);
	}
...
export const getAccount = async function(address) {
	...
	if (value.sequence === undefined || value.sequence === 0 || value.sequence === '') {
		...
		sequence = (0, uint64ToNumber)(value.sequence);
	}
	...
	return {
		...
		balance: 0,
		pubkey: (0, normalizePubkey)(value.public_key) || undefined,
		accountNumber: (0, uint64ToNumber)(value.account_number),
		sequence: sequence,
	};
};
