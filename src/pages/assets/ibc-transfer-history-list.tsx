...
import { colorGold } from 'src/emotion-styles/colors';
import { LinkIcon } from 'src/pages/assets/components/link-icon';
...
function StatusTableData({ status }: { status?: IBCTransferHistoryStatus }) {
	...
	switch (status) {
		case 'complete':
			return (
				<>
					<StatusImg alt="success" src="/public/assets/icons/toast-success.png" />
					Success
				</>
			);
		case 'pending':
		...
		case 'refunded':
			return (
				<>
					<StatusImg alt="failed" src="/public/assets/icons/failed-tx.png" />
					Refunded
				</>
			);
		case 'timeout':
