...
import { setClaimRewardsValidator } from 'src/actions/stake';
				...
				props.rewards.rewards.map((item, index) => {
					...
					return (
						<MenuItem key={item.validator_address} value={item.validator_address}>
							...
							{props.validatorList &&
								props.validatorList.map(value => {
									const rewards =
										item.reward && item.reward[0] && item.reward[0].amount
											? item.reward[0].amount / 10 ** config.COIN_DECIMALS
											: 0;
									...
									if (value.operator_address === item.validator_address) {
										return (
											<span key={value.operator_address}>
												...
												{rewards && rewards > 0 ? <b>&nbsp;({rewards.toFixed(4)})</b> : null}
											</span>
										);
									}
