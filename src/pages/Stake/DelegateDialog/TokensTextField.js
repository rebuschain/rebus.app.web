...
import { setTokens } from 'src/actions/stake';
...
const TokensTextField = props => {
	...
	if (props.selectedValidator && (props.name === 'Undelegate' || props.name === 'Redelegate')) {
		const filterList = props.delegations.find(
			value => value.delegation && value.delegation.validator_address === props.selectedValidator
		);
	...
	return (
		<>
			<TextField
				error={
					(props.name === 'Delegate' || props.name === 'Stake') && vestingTokens
						...
						? props.value > parseFloat(stakedTokens)
						: false
				}
			...
			{vestingTokens && (props.name === 'Delegate' || props.name === 'Stake') ? (
				<div className="available_tokens">
					<p className="heading">{variables[props.lang]['vesting_tokens']}:</p>
