...
export const InsyncWrapper: FunctionComponent<React.PropsWithChildren<{ fullHeight?: boolean }>> = observer(
	({ children, fullHeight }) => {
		...
		return (
			<div
				className="of_community flex flex-col"
				style={{ height: fullHeight ? '100%' : 'fit-content', width: '100%' }}>
				{children}
				<SnackbarMessage />
			</div>
		);
