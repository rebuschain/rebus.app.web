...
import logo from 'src/assets/OmniFlix.svg';
import './index.scss';
...
import { initializeChain } from 'src/utils/helper';
...
import CopyButton from 'src/components/insync/CopyButton/TextButton';
import variables from 'src/utils/variables';
import { fetchProposalDetails, fetchProposalTally, fetchVoteDetails, getProposals } from 'src/actions/proposals';
...
class NavBar extends Component {
	...
	componentDidMount() {
		...
		if (this.props.proposals && !this.props.proposals.length && !this.props.proposalsInProgress && !this.props.stake) {
	...
	componentDidUpdate(pp, ps, ss) {
		...
		if (pp.address !== this.props.address && pp.address !== '' && this.props.address !== '' && !this.props.stake) {
			this.props.getProposals(result => {
				if (result && result.length) {
					const array = [];
					result.map(val => {
	...
	handleFetch(address) {
		if (this.props.balance && !this.props.balance.length && !this.props.balanceInProgress) {
			this.props.getBalance(address);
		}
		if (this.props.vestingBalance && !this.props.vestingBalance.value && !this.props.vestingBalanceInProgress) {
	...
	handleChain(fetch) {
		initializeChain((error, addressList) => {
			...
			const previousAddress = localStorage.getItem('of_co_address') && decode(localStorage.getItem('of_co_address'));
		...
		return (
			<div
				className={ClassNames(
					'nav_bar padding',
					localStorage.getItem('of_co_address') || this.props.address ? '' : 'disconnected_nav'
				)}>
				<img
					alt="OmniFlix"
					src={logo}
				...
				<div className={ClassNames('right_content', this.props.show ? 'show' : '')}>
					...
					{(localStorage.getItem('of_co_address') || this.props.address) && (
						<div className="select_fields">
							...
							<CopyButton data={this.props.address}>{variables[this.props.lang].copy}</CopyButton>
						</div>
					)}
					{localStorage.getItem('of_co_address') || this.props.address ? (
						<DisconnectButton />
