...
describe('Test Rebus Join Pool Tx', () => {
	...
	test('Join Pool with no max slippage', async () => {
		...
		const tx = await new Promise<any>(resolve => {
			account.rebus.sendJoinPoolMsg(poolId, shareOutAmount, '0', '', tx => {
				resolve(tx);
			});
		});
		...
		deepContained(
			{
				type: 'message',
				attributes: [
					{ key: 'action', value: '/rebus.gamm.v1beta1.MsgJoinPool' },
	...
	test('Join Pool with slippage', async () => {
		...
		const tx = await new Promise<any>(resolve => {
			account.rebus.sendJoinPoolMsg(poolId, shareOutAmount, '0.1', '', tx => {
				resolve(tx);
			});
		});
