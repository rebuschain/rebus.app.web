...
describe('Test Rebus Swap Exact Amount Out Tx', () => {
	...
	beforeEach(async () => {
		...
		await new Promise<any>(resolve => {
			account.rebus.sendCreatePoolMsg(
				'0',
				[
					{
						weight: '100',
	...
	test('SwapExactAmountOut should fail with unregistered pool asset', async () => {
		...
		await expect(
			account.rebus.sendSwapExactAmountOutMsg(
				'1',
				{
					coinDenom: 'BAR',
	...
	test('SwapExactAmountOut with no max slippage', async () => {
		...
		const queryPool = queriesStore.get(chainStore.current.chainId).rebus.queryGammPools.getObservableQueryPool(poolId);
		...
		const tx = await new Promise<any>(resolve => {
			account.rebus.sendSwapExactAmountOutMsg(poolId, tokenInCurrency, tokenOut, '0', '', tx => {
				resolve(tx);
			});
		});
		...
		deepContained(
			{
				type: 'message',
				attributes: [
					{ key: 'action', value: '/rebus.gamm.v1beta1.MsgSwapExactAmountOut' },
	...
	test('SwapExactAmountOut with slippage', async () => {
		...
		const tx = await new Promise<any>(resolve => {
			account.rebus.sendSwapExactAmountOutMsg(poolId, tokenInCurrency, tokenOut, doubleSlippage.toString(), '', tx => {
				resolve(tx);
			});
		});
