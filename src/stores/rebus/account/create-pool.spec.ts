...
describe('Test Rebus Create Pool Tx', () => {
	...
	test('CreatePool should fail with 0 assets', async () => {
		...
		await expect(account.rebus.sendCreatePoolMsg('0', [], '', _ => {})).rejects.not.toBeNull();
	});
		...
		deepContained(
			{
				type: 'message',
				attributes: [
					{ key: 'action', value: '/rebus.gamm.v1beta1.MsgCreateBalancerPool' },
	...
	test('Test CreatePool msg with none zero swap fee', async () => {
		...
		const queryPools = queriesStore.get(chainStore.current.chainId).rebus.queryGammPools;
