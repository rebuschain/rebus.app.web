...
describe('Test Rebus Swap Exact Amount In Tx', () => {
	...
	test('SwapExactAmountIn should fail with unregistered pool asset', async () => {
		...
		await expect(
			account.rebus.sendSwapExactAmountInMsg(
				'1',
				{
					currency: {
						coinDenom: 'ATOM',
	...
	test('SwapExactAmountIn with no max slippage', async () => {
		...
		const tx = await new Promise<any>(resolve => {
			account.rebus.sendSwapExactAmountInMsg(poolId, tokenIn, tokenOutCurrency, '0', '', {}, {}, tx => {
				resolve(tx);
			});
		});
		...
		deepContained(
			{
				type: 'message',
				attributes: [
					{ key: 'action', value: '/rebus.gamm.v1beta1.MsgSwapExactAmountIn' },
