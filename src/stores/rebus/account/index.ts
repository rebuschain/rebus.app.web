...
export class AccountWithCosmosAndRebus
	...
	static readonly defaultMsgOpts: CosmosMsgOpts & RebusMsgOpts & CosmwasmMsgOpts = deepmerge(
		...
		deepmerge(AccountWithCosmwasm.defaultMsgOpts, {
			createPool: {
				type: 'rebus/gamm/create-balancer-pool',
				gas: 350000,
			},
			joinPool: {
				type: 'rebus/gamm/join-pool',
				gas: 240000,
				shareCoinDecimals: 18,
			},
			joinSwapExternAmountIn: {
				type: 'rebus/gamm/join-swap-extern-amount-in',
				gas: 140000,
				shareCoinDecimals: 18,
			},
			exitPool: {
				type: 'rebus/gamm/exit-pool',
				gas: 140000,
				shareCoinDecimals: 18,
			},
			swapExactAmountIn: {
				type: 'rebus/gamm/swap-exact-amount-in',
				gas: 250000,
			},
			swapExactAmountOut: {
				type: 'rebus/gamm/swap-exact-amount-out',
				gas: 250000,
			},
			lockTokens: {
				type: 'rebus/lockup/lock-tokens',
				gas: 450000,
			},
			superfluidDelegate: {
				type: 'rebus/superfluid-delegate',
				gas: 500000,
			},
			lockAndSuperfluidDelegate: {
				type: 'rebus/lock-and-superfluid-delegate',
				gas: 500000,
			},
			beginUnlocking: {
				type: 'rebus/lockup/begin-unlock-period-lock',
				// Gas per msg
				gas: 140000,
			},
			unlockPeriodLock: {
				type: 'rebus/lockup/unlock-period-lock',
				// Gas per msg
				gas: 140000,
			},
			superfluidUndelegate: {
				type: 'rebus/superfluid-undelegate',
				gas: 300000,
			},
			superfluidUnbondLock: {
				type: 'rebus/superfluid-unbond-lock',
				// Gas per msg
				gas: 300000,
			},
		})
...
export class RebusAccount {
	...
	async sendCreatePoolMsg(
		...
		await this.base.sendMsgs(
			'createPool',
			{
				...
				protoMsgs: [
					{
						type_url: '/rebus.gamm.v1beta1.MsgCreateBalancerPool',
						value: rebus.gamm.v1beta1.MsgCreateBalancerPool.encode({
	...
	async sendJoinPoolMsg(
		...
		await this.base.sendMsgs(
			'joinPool',
			async () => {
				const queryPool = queries.rebus.queryGammPools.getObservableQueryPool(poolId);
				...
				return {
					...
					protoMsgs: [
						{
							type_url: '/rebus.gamm.v1beta1.MsgJoinPool',
							value: rebus.gamm.v1beta1.MsgJoinPool.encode({
								sender: msg.value.sender,
			...
			tx => {
				if (tx.code == null || tx.code === 0) {
					...
					queries.rebus.queryGammPools.getObservableQueryPool(poolId).fetch();
				}
		...
		await this.base.sendMsgs(
			...
			async () => {
				...
				return {
					...
					protoMsgs: [
						{
							type_url: '/rebus.gamm.v1beta1.MsgJoinSwapExternAmountIn',
							value: rebus.gamm.v1beta1.MsgJoinSwapExternAmountIn.encode({
								sender: msg.value.sender,
								poolId: Long.fromString(msg.value.poolId),
		...
		await this.base.sendMsgs(
			...
			async () => {
				...
				for (const route of routes) {
					const queryPool = queries.rebus.queryGammPools.getObservableQueryPool(route.poolId);
					await queryPool.waitFreshResponse();
				...
				return {
					...
					protoMsgs: [
						{
							type_url: '/rebus.gamm.v1beta1.MsgSwapExactAmountIn',
							value: rebus.gamm.v1beta1.MsgSwapExactAmountIn.encode({
								sender: msg.value.sender,
			...
			tx => {
				if (tx.code == null || tx.code === 0) {
					...
					// Refresh the pools
					for (const route of routes) {
						queries.rebus.queryGammPools.getObservableQueryPool(route.poolId).fetch();
					}
				}
		...
		await this.base.sendMsgs(
			...
			async () => {
				...
				return {
					...
					protoMsgs: [
						{
							type_url: '/rebus.gamm.v1beta1.MsgSwapExactAmountOut',
							value: rebus.gamm.v1beta1.MsgSwapExactAmountOut.encode({
								sender: msg.value.sender,
		...
		await this.base.sendMsgs(
			...
			async () => {
				...
				return {
					...
					protoMsgs: [
						{
							type_url: '/rebus.gamm.v1beta1.MsgExitPool',
							value: rebus.gamm.v1beta1.MsgExitPool.encode({
								sender: msg.value.sender,
								poolId: Long.fromString(msg.value.poolId),
	...
	async sendLockTokensMsg(
		...
		await this.base.sendMsgs(
			'lockTokens',
			{
				...
				protoMsgs: [
					{
						type_url: '/rebus.lockup.MsgLockTokens',
						value: rebus.lockup.MsgLockTokens.encode({
			...
			tx => {
				if (tx.code == null || tx.code === 0) {
					...
					queries.rebus.queryLockedCoins.get(this.base.bech32Address).fetch();
					queries.rebus.queryAccountLocked.get(this.base.bech32Address).fetch();

					queries.rebus.querySuperfluidDelegations.getQuerySuperfluidDelegations(this.base.bech32Address).fetch();
				}
	...
	async sendSuperfluidDelegate(
		...
		const protoMsgs = msgs.map(msg => {
			return {
				type_url: '/rebus.superfluid.MsgSuperfluidDelegate',
				value: rebus.superfluid.MsgSuperfluidDelegate.encode({
					sender: msg.value.sender,
	...
	async sendLockAndSuperfluidDelegateMsg(
		...
		await this.base.sendMsgs(
			'lockAndSuperfluidDelegate',
			{
				...
				protoMsgs: [
					{
						type_url: '/rebus.superfluid.MsgLockAndSuperfluidDelegate',
						value: rebus.superfluid.MsgLockAndSuperfluidDelegate.encode({
							sender: msg.value.sender,
							coins: msg.value.coins,
							valAddr: msg.value.val_addr,
						}).finish(),
	...
	async sendBeginUnlockingMsg(lockIds: string[], memo: string = '', onFulfill?: (tx: any) => void) {
		...
		const protoMsgs = msgs.map(msg => {
			return {
				type_url: '/rebus.lockup.MsgBeginUnlocking',
				value: rebus.lockup.MsgBeginUnlocking.encode({
					owner: msg.value.owner,
		...
		await this.base.sendMsgs(
			...
			tx => {
				if (tx.code == null || tx.code === 0) {
					...
					// Refresh the locked coins
					queries.rebus.queryLockedCoins.get(this.base.bech32Address).fetch();
					queries.rebus.queryUnlockingCoins.get(this.base.bech32Address).fetch();
					queries.rebus.queryAccountLocked.get(this.base.bech32Address).fetch();
				}
	...
	async sendBeginUnlockingMsgOrSuperfluidUnbondLockMsgIfSyntheticLock(
		...
		const protoMsgs = msgs.map(msg => {
			if (msg.type === this.base.msgOpts.beginUnlocking.type && msg.value.ID) {
				...
				return {
					type_url: '/rebus.superfluid.MsgSuperfluidUndelegate',
					value: rebus.superfluid.MsgSuperfluidUndelegate.encode({
						sender: msg.value.sender,
						lockId: Long.fromString(msg.value.lock_id),
					}).finish(),
				};
				...
				return {
					type_url: '/rebus.superfluid.MsgSuperfluidUnbondLock',
					value: rebus.superfluid.MsgSuperfluidUnbondLock.encode({
						sender: msg.value.sender,
						lockId: Long.fromString(msg.value.lock_id),
					}).finish(),
				};
		...
		await this.base.sendMsgs(
			...
			tx => {
				if (tx.code == null || tx.code === 0) {
					...
					queries.rebus.querySuperfluidDelegations.getQuerySuperfluidDelegations(this.base.bech32Address).fetch();
					queries.rebus.querySuperfluidUndelegations.getQuerySuperfluidDelegations(this.base.bech32Address).fetch();
				}
